...
if (isset($_POST['submit'])) {
    ...
    $checkPastScore = "SELECT * FROM snakedb WHERE username = '$playername'";
    $checkResult = mysqli_query($conn,$checkPastScore);
    if ($checkResult -> num_rows >0){
      $updateQuery = "UPDATE snakedb SET score=$score where username = '$playername'";
      $conn->query($updateQuery);
      echo "Score updated successfully for $playername";
      header("Location: http://localhost/comproject/snake.php");
      exit();
    } else {
      ...
      $insert = "INSERT INTO snakedb (username, usertag, score)
      VALUES ('$playername','$usertag','$score');";
      $check =  mysqli_query($conn,$insert);
      if ($check){
        echo "Score Registered Successfully!";
        header("Location: http://localhost/comproject/games/snake.php");
        exit();
        } else {
